...
//			cout << temp1 << " " << temp2 << endl;
		...
		for(int j=0; j<N;j++){
			...
			// polar-log scale coordinates to cartesian
			//data_t mag = pow(10, dout_mag[j]/10);	//mag sqrd
			/*data_t real_comp = mag*cos(dout_angle[j]);
			data_t imag_comp = mag*sin(dout_angle[j]);
			data_out[j] = data_comp(real_comp, imag_comp);*/
