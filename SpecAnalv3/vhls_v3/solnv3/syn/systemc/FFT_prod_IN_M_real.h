...
#ifndef _FFT_prod_IN_M_real_H_
#define _FFT_prod_IN_M_real_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "FFT_prod_IN_M_real_memcore.h"

SC_MODULE(FFT_prod_IN_M_real) {
...
protected:
    ...
    // buffer instances
    FFT_prod_IN_M_real_memcore* buffer[BufferCount];
...
public:
    ~FFT_prod_IN_M_real();
    SC_CTOR(FFT_prod_IN_M_real) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "FFT_prod_IN_M_real_memcore_U_";
            name += szTmp;

            buffer[i] = new FFT_prod_IN_M_real_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "FFT_prod_IN_M_real.trace");
            // ports
