...
#ifndef _FFT_xin_M_imag_H_
#define _FFT_xin_M_imag_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "FFT_xin_M_imag_memcore.h"

SC_MODULE(FFT_xin_M_imag) {
...
protected:
    ...
    FFT_xin_M_imag_memcore* FFT_xin_M_imag_memcore_U;
...
public:
    ~FFT_xin_M_imag();
    SC_CTOR(FFT_xin_M_imag) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        FFT_xin_M_imag_memcore_U = new FFT_xin_M_imag_memcore("FFT_xin_M_imag_memcore_U");
        FFT_xin_M_imag_memcore_U->clk      ( clk );
        FFT_xin_M_imag_memcore_U->reset    ( reset );
        FFT_xin_M_imag_memcore_U->ce0      ( i_ce0 );
        FFT_xin_M_imag_memcore_U->we0      ( i_we0 );
        FFT_xin_M_imag_memcore_U->address0 ( memcore_iaddr );
        FFT_xin_M_imag_memcore_U->d0       ( i_d0 );
        FFT_xin_M_imag_memcore_U->q0       ( i_q0 );

        FFT_xin_M_imag_memcore_U->ce1      ( t_ce0 );
        FFT_xin_M_imag_memcore_U->we1      ( t_we0 );
        FFT_xin_M_imag_memcore_U->address1 ( memcore_taddr );
        FFT_xin_M_imag_memcore_U->d1       ( t_d0 );
        FFT_xin_M_imag_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "FFT_xin_M_imag.trace");
            // ports
