...
#ifndef _FFT_data_OUT1_M_ig8j_H_
#define _FFT_data_OUT1_M_ig8j_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "FFT_data_OUT1_M_ig8j_memcore.h"

SC_MODULE(FFT_data_OUT1_M_ig8j) {
...
protected:
    ...
    // buffer instances
    FFT_data_OUT1_M_ig8j_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(FFT_data_OUT1_M_ig8j) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "FFT_data_OUT1_M_ig8j_memcore_U_";
            name += szTmp;

            buffer[i] = new FFT_data_OUT1_M_ig8j_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "FFT_data_OUT1_M_ig8j.trace");
            // ports
